...
class FormOne(QtGui.QWidget):
    ...
    def genPDF(self,isPrint):
        ...
        inData['TransportationCost']=rounder(float(self.textBoxTransAmount.text()))
        ...
        if self.combo_OrdTy.currentText()=="Blocks":
            ...
            self.getO=getOrder(self.combo_OrdTy.currentText(),inData,isPrint)
            self.getO.show()
...
class BlocksOrder(QtGui.QWidget):
    ...
    def genPDF(self,inData,isPrint):
        ...
        outPath=QtGui.QFileDialog.getSaveFileName(self,'Save Pdf','',"PDF files (*.pdf)")
    ...
    def center(self):      # Initialize the window at the centre of the screen
        qr = self.frameGeometry()                                   # Get the position rectangle around our window
        cp = QtGui.QDesktopWidget().availableGeometry().center()    # Get the screen size of the desktop and find its centre
        qr.moveCenter(cp)                                           # Move the position rectangle around our window to the center calculated
...
class ValidatedItemDelegate(QtGui.QStyledItemDelegate):
    def createEditor(self, widget, option, index):
        ...
        if index.column() == 0: #only on the cells in the first column
            ...
            validator = QtGui.QRegExpValidator(QtCore.QRegExp("[\d]{,6}"), editor)
