...
def createBullet(c,text,x,y,font,size,indent=0):
    ...
    style = styles["Normal"]
    if indent!=0:
        t=ListFlowable([Paragraph(text, style)], bulletType='bullet', start='circle',leftIndent = indent,fontName =font,bulletFontName ="Helvetica",bulletFontSize=8,bulletAnchor="Start")
    else:
        t=ListFlowable([Paragraph(text, style)], bulletType='bullet', start='circle',fontName =font,bulletFontName ="Helvetica",bulletFontSize=8,bulletAnchor="Start")
    t.wrapOn(c, 480, 20)
    t.drawOn(c,x,y)
    return y-size
...
def drawTable(c,x,y,data,Textcolor,fontSize,dx=0.6,total=True):
    pading=[max([len(el[index]) for el in data]) for index in xrange(len(data[0]))]
    pading=[pading[index]+8 if index==0 else pading[index]+4 for index in xrange(len(pading))]
    ...
    for m,rec in enumerate(data):
        ...
        createLine(c,x-5,y-10,sum(pading)*fontSize*dx,linewidth,"0xff8100")
        for k,word in enumerate(rec):
            if m==0 or (m==len(data)-1 and total):
                _=createTextBox(c,word,x,y,Textcolor,"MavenProBlack",fontSize)
            else:
                _=createTextBox(c,word,x,y,Textcolor,"MavenPro",fontSize)
...
def getPieChart(c,x,y,data,data_suff,labels,colors,fsize=8.5,radius=0.60,legdx=180,legdy=100):
    ...
    legend.colorNamePairs = legendList # [(HexColor(colors[0]), ('BP')), (HexColor(colors[1]), ('BT'))]
